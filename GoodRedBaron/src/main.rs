...
fn get_process_info(h: HANDLE) -> Result<ProcessInfo> {
    ...
    // grab full path
    unsafe {
        if GetProcessImageFileNameW(h, &mut buffer) == 0 {
            return Err(anyhow!("GetProcessImageFileNameW failed"));
        }
    }
...
fn scan_process_memory(h: HANDLE, pid: u32, scanner: &mut Scanner) -> Result<()> {
    ...
    loop {
        ...
        for rule in scan_results.matching_rules() {
            warn!("rule matched for {}: {}", pi.name, rule.identifier());

            match crate::notifications::notify(&pi.name, &pi.path, &rule) {
                Ok(_) => {}
                Err(e) => {
...
fn main() -> Result<()> {
    ...
    loop {
        ...
        for pid in pids {
            ...
            match scan_process_memory(h, pid, &mut scanner) {
                Ok(_) => {}
                Err(e) => {
